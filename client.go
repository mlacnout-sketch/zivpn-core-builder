...
func initClientFlags() {
	...
	clientCmd.Flags().StringVar(&configContent, "config", "", "JSON config content (ZIVPN)")
	clientCmd.Flags().StringVarP(&obfsFlag, "obfs-password", "s", "", "Obfuscation password (ZIVPN)")
}

// ZIVPN Custom Config Struct to match the flat JSON format
...
func runClient(cmd *cobra.Command, args []string) {
	...
	if configContent != "" {
		...
		// ZIVPN sends obfs as a simple string, which maps to Salamander in Hysteria v1
		if zConf.Obfs != "" {
			config.Obfs.Type = "salamander"
			config.Obfs.Salamander.Password = zConf.Obfs
		}
		
		// Override Obfs if flag is present (Common pattern in ZIVPN Turbo)
		if obfsFlag != "" {
			config.Obfs.Type = "salamander"
			config.Obfs.Salamander.Password = obfsFlag
		}

		fmt.Println("ZIVPN UDP running")
	} else {
		// Standard Hysteria Logic
